...
(function ($, CONFIG) {
        ...
        switch (type) {
            ...
            case 'PAGES':
                $searchItems = array.map(function (item) {
                    ...
                    return searchItem('file', item.title, null, item.excerpt.slice(0, 150), CONFIG.ROOT_URL + item.path);
                });
                break;
            case 'CATEGORIES':
            case 'TAGS':
        ...
        var containKeywords = keywordArray.filter(function (keyword) {
            var containFields = fields.filter(function (field) {
                if (!obj.hasOwnProperty(field))
                    return false;
                if (bodauTiengViet(obj[field]).toUpperCase().indexOf(keyword) > -1)
                    return true;
            });
    ...
    $.getJSON(CONFIG.CONTENT_URL, function (json) {
        ...
        $input.on('input', function () {
            var keywords = bodauTiengViet($.trim($(this).val()));
